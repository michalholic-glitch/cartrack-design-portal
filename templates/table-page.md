...
AppBar (title + search + one primary action) > filter row (Chips) > DataTable > Pagination

## Components used

- AppBar — page title, global search, ONE contained primary action (e.g. "Add vehicle"), overflow menu
...
- AppBar (contextual) — swaps in when rows are selected: count + bulk actions
- Menu — per-row overflow actions (⋮), destructive actions in error colour with confirmation Dialog

## Structure rules

- One primary action per page, hosted in the AppBar — never a FAB on desktop.
- Filters live between the bar and the table, not in a sidebar, unless there are more than ~8 filters (then SideSheet).
...
- Row click opens the detail page; row checkbox enters selection mode (contextual AppBar).
