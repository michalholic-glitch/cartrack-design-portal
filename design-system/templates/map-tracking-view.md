...
NavigationRail (left edge) > full-bleed map canvas > floating asset panel (docked Drawer, left) > floating controls (right)

## Components used

- NavigationRail — primary app navigation on the left edge; the map never hides it
- Drawer (as docked panel) — searchable asset list synced with the map; selecting an asset focuses the map and vice versa
